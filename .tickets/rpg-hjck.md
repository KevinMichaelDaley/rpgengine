...
**2026-07-19T21:27:45Z**

DESIGN DOC: ref/gi_streaming_design.md now captures the client GI/light-data streaming architecture (zones->chunks two-level streaming via fr_asset_stream; dual-index visibility prepass for independent SDF-voxel vs lightmap-atlas chunk boundaries; job-fiber load + GPU_CMD_CUSTOM upload separation; decoupled from descriptor load). New children: rpg-oda7 (scaffold, FIRST), rpg-sazm (dual prepass), rpg-jro2 (exporter lightmap_unpack + chunked bake), rpg-s720 (zones), rpg-c7fk (gi_runtime external SDF residency), rpg-rbon (STREAM_PRIORITY wiring).

**2026-07-19T22:24:49Z**

Added rpg deterministic/optimized lightmap-unwrap ticket (rpg-5qjv): current pipeline relies on lightmap_pack determinism (worked around by always re-baking after re-export).
