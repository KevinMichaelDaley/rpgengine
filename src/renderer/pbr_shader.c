...
static const char *const PBR_FS =
    ...
    "  return lit / 9.0;\n"
    "}\n"
    /* PCSS soft sun shadow on plain linear-depth cascades. Cascades TILE the\n"
     * light frustum, so a fragment lands in one tile (or two across a seam); we\n"
     * sample EVERY cascade it lands in and union the occlusion (min). Per cascade:\n"
     * (1) blocker search over\n"
    ...
    /* SOFT depth gate: the old binary (d <= md) step flipped whole taps at the\n"
     * glass boundary -- with the per-pixel tap rotation that read as speckle.\n"
     ...
     * the opaque penumbra radius), averaged -- decoupled from the binary opaque\n"
     * PCF, whose per-tap noise it used to inherit. Multiplying the opaque lit\n"
     * fraction by this smooth field keeps the colored shadow's penumbra without\n"
     * the boundary speckle; where frame shadow overlaps glass, dark x tint is\n"
     * still dark. */
    "vec3 pbr_csm_glass(vec2 uv, int i, float d, float gbias, float prad){\n"
    "  if(u_csm_mask_on==0) return vec3(1.0);\n"
    /* DETERMINISTIC tent: bilinear mask + smooth gate leave nothing to dither\n"
     * -- a per-pixel-rotated disk here re-introduced boundary noise (variance\n"
     * localised exactly at the glass silhouette). Fixed cross at ~1.3 texels\n"
     * (clamped fraction of the opaque penumbra) softens the silhouette without\n"
     * any per-pixel randomness. */
    "  float r = min(prad*0.5, 2.6/u_csm_res);\n"
    "  vec3 t = pbr_csm_tap_trans(uv, i, d, gbias) * 0.4;\n"
    "  t += pbr_csm_tap_trans(uv + vec2( r, 0.0), i, d, gbias) * 0.15;\n"
    "  t += pbr_csm_tap_trans(uv + vec2(-r, 0.0), i, d, gbias) * 0.15;\n"
    "  t += pbr_csm_tap_trans(uv + vec2(0.0,  r), i, d, gbias) * 0.15;\n"
    "  t += pbr_csm_tap_trans(uv + vec2(0.0, -r), i, d, gbias) * 0.15;\n"
    "  return t;\n"
    "}\n"
     ...
     * per-tap merge was architecturally right but sampled the discontinuous\n"
     * mask point-wise through the same rotated disk -- binomial speckle at\n"
