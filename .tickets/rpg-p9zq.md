...
Epic for the LuaJIT scripting system with dedicated thread architecture. The script runtime runs on its own pthread, reads entity state from a frozen snapshot, writes updates via a double-buffered blob, and supports both Lua and native C code through a unified script_env_t interface.
...
8. rpg-96oo — Lua entity manipulation API bindings
9. rpg-7x16 — Math/vec3/quat Lua bindings
...
- Native C functions use same script_env_t interface as Lua
