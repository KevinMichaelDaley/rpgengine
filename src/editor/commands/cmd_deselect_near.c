/**
 * @file cmd_deselect_near.c
 * @brief Deselect entities within a distance of a point.
 *
 * JSON args: {"pos": [x,y,z], "dist": r}
 * If "pos" is omitted, uses the position of the entity named "@cursor".
 * Removes from selection all active entities within Euclidean distance r.
...
bool cmd_deselect_near(edit_dispatch_t *d, const json_value_t *args,
    ...
    /* Iterate all entities, deselect those within distance. */
    uint32_t removed = 0;
