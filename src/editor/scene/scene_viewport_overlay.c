...
/* Selection highlighting is now handled via fragment shader tinting
 * in viewport_render_draw_entities(). This function is retained as a
...
void viewport_render_draw_selection_outline(viewport_render_state_t *state,
    ...
    (void)state; (void)entities; (void)selection; (void)active_object_id;
    (void)view; (void)proj; (void)eye_pos; (void)fov_y; (void)fbo_height;
    (void)shading_mode;
}
