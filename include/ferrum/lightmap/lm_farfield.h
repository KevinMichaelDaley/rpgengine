 ...
 * ignored so the near-field solver's contribution is not double-counted.
 *
 * Ownership: none -- writes into @p lm's luxel SH. Nullability: @p lm, @p svo,
 * @p table non-NULL. Errors: none. Side effects: accumulates into luxel SH
 * (run once, after/alongside the direct pass; the solver reads the seeded SH).
 */
 ...
 *        @p table. @p max_dist bounds the ray length; @p seed makes the
 *        stochastic hemisphere sampling deterministic.
 */
