...
static const char *const PBR_FS =
    ...
    "  for(int k=0;k<9;++k){ shr[k]=0.0; shg[k]=0.0; shb[k]=0.0; }\n"
    "  int ni[3]; float ne[3]; ni[0]=n0;ni[1]=n1;ni[2]=n2; ne[0]=e0;ne[1]=e1;ne[2]=e2;\n"
    "  float wsum=0.0;\n"
    "  for(int j=0;j<3;++j){ if(ni[j]<0) continue; float w=1.0/(ne[j]+1e-4); wsum+=w;\n"
    "    int base=ni[j]*27;\n"
    "    for(int k=0;k<9;++k){ shr[k]+=texelFetch(u_probe_sh,base+k).r*w;\n"
    "      shg[k]+=texelFetch(u_probe_sh,base+9+k).r*w;\n"
    "      shb[k]+=texelFetch(u_probe_sh,base+18+k).r*w; }\n"
    "  }\n"
    "  if(wsum<=0.0) return vec3(0.0);\n"
    "  float inv=1.0/wsum; vec3 nn=normalize(N);\n"
    "  float y0=0.282094792, y1=0.488602512*nn.y, y2=0.488602512*nn.z, y3=0.488602512*nn.x;\n"
    "  float y4=1.092548431*nn.x*nn.y, y5=1.092548431*nn.y*nn.z, y6=0.315391565*(3.0*nn.z*nn.z-1.0);\n"
    "  float y7=1.092548431*nn.x*nn.z, y8=0.546274215*(nn.x*nn.x-nn.y*nn.y);\n"
    "  vec3 r;\n"
    "  r.r=3.14159265*shr[0]*y0+2.0943951*(shr[1]*y1+shr[2]*y2+shr[3]*y3)+0.7853982*(shr[4]*y4+shr[5]*y5+shr[6]*y6+shr[7]*y7+shr[8]*y8);\n"
    "  r.g=3.14159265*shg[0]*y0+2.0943951*(shg[1]*y1+shg[2]*y2+shg[3]*y3)+0.7853982*(shg[4]*y4+shg[5]*y5+shg[6]*y6+shg[7]*y7+shg[8]*y8);\n"
    "  r.b=3.14159265*shb[0]*y0+2.0943951*(shb[1]*y1+shb[2]*y2+shb[3]*y3)+0.7853982*(shb[4]*y4+shb[5]*y5+shb[6]*y6+shb[7]*y7+shb[8]*y8);\n"
