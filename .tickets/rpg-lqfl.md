...
Implement native C function registration for the script runtime — C functions that run on the script thread using the same script_env_t interface as Lua scripts.
...
- Native functions execute every tick on the script thread, after Lua scripts
- Native functions use the same script_env_t reads/writes as Lua
