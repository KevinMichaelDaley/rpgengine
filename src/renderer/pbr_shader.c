...
static const char *const PBR_FS =
    ...
    /* Current-pass punctual lights (point / directional / spot). */
    "  for(int i=0; i<u_light_count && i<PBR_MAX_LIGHTS; ++i){\n"
    "    vec3 L; float atten = 1.0;\n"
    "    if(u_light_type[i]==1){ L = normalize(-u_light_dir[i]); }\n"
    "    else {\n"
    "      vec3 d = u_light_pos[i] - v_world_pos; float dist = length(d);\n"
    "      L = d / max(dist, 1e-4); atten = pbr_atten(dist, u_light_range[i]);\n"
    "      if(u_light_type[i]==2){\n"
    "        float c = dot(-L, normalize(u_light_dir[i]));\n"
    "        atten *= smoothstep(u_light_cos_outer[i], u_light_cos_inner[i], c);\n"
    "      }\n"
    "    }\n"
    "    direct += pbr_light(N, V, L, albedo, rough, metal, F0) * u_light_color[i] * atten;\n"
    "  }\n"
    "  vec3 ambient;\n"
    ...
    "  if(u_has_emissive==1) emissive *= texture(u_emissive_map,v_uv0).rgb;\n"
    "  color += emissive;\n"
