/**
 * @file cmd_redo.c
 * @brief Redo command — re-applies the last undone operation (or group).
 *
...
/** @brief Maximum entities affected by a single redo step. */
#define REDO_MAX_AFFECTED 64
...
bool cmd_redo(edit_dispatch_t *d, const json_value_t *args,
    ...
    if (edit_undo_redo_count(ctx->undo) == 0) {
        result->type   = JSON_NUMBER;
        result->number = 0.0;
        return true;
    }
    ...
    uint32_t redone = edit_undo_redo(ctx->undo);
    if (redone == 0) {
        result->type   = JSON_NUMBER;
        result->number = 0.0;
        return true;
    ...
    for (uint32_t i = cursor_before; i < cursor_after; i++) {
        ...
        edit_undo_apply_forward(ctx, entry);
    }
