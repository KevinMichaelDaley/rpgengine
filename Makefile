...
# OpenImageDenoise (baked-lightmap denoise). The real library is heavy to build
# (ISPC + TBB), so it is opt-in via OIDN=1; the default build uses a no-op stub
# so headless/CI compiles without the OIDN toolchain. Exactly one variant is
# compiled — filter both out of the wildcard, then add the selected one back.
OIDN ?= 0
...
SRC_HEADLESS := $(JOB_SRC) $(MATH_SRC) $(MEM_SRC) $(ECS_SRC) $(ENTITY_SRC) $(NET_SRC) $(SERVER_SRC) $(PHYS_SRC) $(MESH_SRC) $(SCENE_DESC_SRC) $(PROBE_SRC) $(ASSET_STREAM_SRC) $(ENGINE_SRC) $(EDITOR_SRC) $(ASSET_SRC) $(AEGIS_SRC) $(LLM_SRC) $(ANIM_SRC) $(NPC_SRC) $(PROCGEN_SRC) $(LIGHTMAP_SRC) $(RENDERER_DEBUG_LINES_SRC)
...
BIN_HEADLESS += build/probe_brick_file_tests
...
# Scene/level descriptor loader (rpg-51nf). Headless: JSON parser + arena only,
# so it links against nothing but libm (no faiss/openblas from $(LDFLAGS)).
...
# Global low-res zone SDF compose (page-fault fallback for probe rays). Headless.
build/gi_zone_sdf_tests: tests/renderer/gi_zone_sdf_tests.c src/renderer/gi/gi_zone_sdf_compose.c | build
	$(CC) $(CFLAGS) tests/renderer/gi_zone_sdf_tests.c src/renderer/gi/gi_zone_sdf_compose.c -o $@ -lm
...
# GL internal-format / compressed-format enum value-check (rpg-1066). Pure
# header include -- no GL context, no sources.
build/gl_constants_tests: tests/renderer/gl_constants_tests.c include/ferrum/renderer/gl_constants.h | build
	$(CC) $(CFLAGS) tests/renderer/gl_constants_tests.c -o $@
...
build/probe_fixup_tests: tests/probe/probe_fixup_tests.c src/probe/place/probe_fixup_apply.c | build
	$(CC) $(CFLAGS) tests/probe/probe_fixup_tests.c src/probe/place/probe_fixup_apply.c -o $@ -lm
...
PROBE_BAKE_PLACE_SRC := src/probe/place/probe_brick_place.c src/probe/place/probe_fixup_apply.c \
    ...
    src/probe/place/probe_brick_file.c src/probe/place/probe_brick_index_build.c \
...
build/probe_brick_file_tests: tests/probe/probe_brick_file_tests.c src/probe/place/probe_brick_file.c src/memory/arena_init.c src/memory/arena_alloc.c | build
	$(CC) $(CFLAGS) tests/probe/probe_brick_file_tests.c src/probe/place/probe_brick_file.c src/memory/arena_init.c src/memory/arena_alloc.c -o $@ -lm
...
build/asset_stream_tests: tests/asset/asset_stream_tests.c $(ASSET_STREAM_SRC) $(PROBE_SRC) $(JOB_SRC) $(MEM_SRC) | build
	$(CC) $(CFLAGS) tests/asset/asset_stream_tests.c $(ASSET_STREAM_SRC) $(PROBE_SRC) $(JOB_SRC) $(MEM_SRC) -o $@ -lm

# Canonical collider primitive + BODY_SPAWN schema + converters (rpg-b5r3).
COLLIDER_PRIM_TEST_SRC := src/net/replication/body_spawn.c src/net/replication/body_spawn_collider.c src/net/replication/quat_smallest3.c src/net/quantization/float16.c src/scene/scene_desc_collider_prim.c
build/collider_prim_tests: tests/asset/collider_prim_tests.c $(COLLIDER_PRIM_TEST_SRC) | build
...
# Server-assigned streaming priority protocol (rpg-3ldk). Headless.
build/stream_priority_tests: tests/net/stream_priority_tests.c src/net/replication/stream_priority.c src/net/replication/stream_priority_apply.c src/server/level/server_stream_priority.c $(ASSET_STREAM_SRC) $(JOB_SRC) $(MEM_SRC) | build
	$(CC) $(CFLAGS) tests/net/stream_priority_tests.c src/net/replication/stream_priority.c src/net/replication/stream_priority_apply.c src/server/level/server_stream_priority.c $(ASSET_STREAM_SRC) $(JOB_SRC) $(MEM_SRC) -o $@ -lm
...
# --- rpg-bpiz: GPU voxel rasterizer (needs a GPU; SKIPs itself without EGL) ---
build/lm_gpu_voxelize_tests: tests/lightmap/lm_gpu_voxelize_tests.c src/lightmap/gpu/lm_gpu_voxelize.c src/lightmap/gpu/lm_gpu_voxelize_util.c src/lightmap/gpu/lm_gpu_voxvol.c src/lightmap/gpu/lm_gpu_voxraster.c src/lightmap/gpu/lm_gpu_voxsample.c src/lightmap/gpu/lm_gpu_voxscene.c src/renderer/egl_headless.c src/lightmap/gpu/lm_gpu_chunk_build.c src/lightmap/lm_chunk_svo.c src/lightmap/lm_svo_voxelize.c src/lightmap/lm_svo_material.c src/lightmap/lm_svo_mip.c src/lightmap/lm_material.c src/lightmap/lm_image.c src/npc/nav/npc_svo_init.c src/npc/nav/npc_svo_rasterize.c src/npc/nav/npc_svo_blocker.c src/math/vec3.c include/ferrum/lightmap/gpu/lm_gpu_voxelize.h | build
	$(CC) $(CFLAGS) -D_POSIX_C_SOURCE=200809L tests/lightmap/lm_gpu_voxelize_tests.c src/lightmap/gpu/lm_gpu_voxelize.c src/lightmap/gpu/lm_gpu_voxelize_util.c src/lightmap/gpu/lm_gpu_voxvol.c src/lightmap/gpu/lm_gpu_voxraster.c src/lightmap/gpu/lm_gpu_voxsample.c src/lightmap/gpu/lm_gpu_voxscene.c src/renderer/egl_headless.c src/lightmap/gpu/lm_gpu_chunk_build.c src/lightmap/lm_chunk_svo.c src/lightmap/lm_svo_voxelize.c src/lightmap/lm_svo_material.c src/lightmap/lm_svo_mip.c src/lightmap/lm_material.c src/lightmap/lm_image.c src/npc/nav/npc_svo_init.c src/npc/nav/npc_svo_rasterize.c src/npc/nav/npc_svo_blocker.c src/math/vec3.c -o $@ -lEGL -lGL -ldl -lm

# --- rpg-fzht: scene chunk grid (headless geometry unit test) ---
CHUNK_GRID_SRC := src/renderer/chunk/chunk_grid_init.c src/lightmap/lm_chunk_svo.c src/renderer/chunk/chunk_grid_index.c src/renderer/chunk/chunk_grid_bounds.c
...
test: $(BIN_HEADLESS) build/p008_net_replication_protocol_tests build/p000_job_queue_sharding_tests build/p000_job_queue_diagnostics_tests build/p000_ws_deque_tests build/p007_net_client_rx_tests build/p007_net_client_rx_udp_topic_tests build/p007_net_topic_dispatch_tests build/npc_kg_astar_tests build/npc_kg_spatial_tests build/scene_desc_tests build/render_config_tests build/gi_zone_sdf_tests build/gl_constants_tests build/frustum_cull_tests build/probe_tests build/asset_stream_tests build/collider_prim_tests build/gi_collider_pose_tests build/server_level_load_tests build/stream_priority_tests
...
build/p004_visual_hall_forward: tests/visual/p004_visual_hall_forward.c src/renderer/forward_plus.c src/renderer/cluster_grid.c src/renderer/render_camera.c src/renderer/render_scene.c $(RENDER_WORLD_SRC) src/renderer/gi/gi_runtime.c src/renderer/gi/gi_brick_gpu.c src/renderer/gi/gi_runtime_bricks.c src/renderer/gi/gi_probe_gpu_dyn.c src/renderer/gi/gi_runtime_static.c src/renderer/gi/gi_static_volume.c src/renderer/gi/gi_vis_prepass.c src/renderer/gi/gi_sdf_stream.c src/renderer/gi/gi_probe_gpu.c src/renderer/gi/gi_probe_gpu_active.c src/renderer/gi/gi_probe_tuning.c src/renderer/gi/gi_probe_set.c src/renderer/gi/gi_probe_grid.c src/renderer/gi/gi_probe_place.c src/renderer/gi/gi_sdf.c src/lightmap/lm_sdf_file.c src/renderer/light_store.c src/mesh/obj_mesh_load.c src/renderer/mesh/static_mesh_create.c src/renderer/mesh/static_mesh_destroy.c src/renderer/mesh/static_mesh_draw.c src/renderer/vao_create.c src/renderer/vao_bind_attributes.c src/renderer/vao_destroy.c src/renderer/vbo_create.c src/renderer/vbo_upload.c src/renderer/vbo_destroy.c src/renderer/pbr_shader.c src/renderer/material.c src/renderer/texture_create.c src/renderer/texture_upload.c src/renderer/texture_bind.c src/renderer/shader_program_create.c src/renderer/shader_program_bind.c src/renderer/shader_program_destroy.c src/renderer/shader_program_handle.c src/renderer/shader_uniforms_init.c src/renderer/gl_loader_validate.c src/math/mat4_proj.c src/math/mat4_look_at.c src/math/mat4_mul.c src/math/mat4_basic.c src/math/vec3.c $(OBJ_GLAD) | build
...
build/lm_mesh_bake_tests: tests/lightmap/lm_mesh_bake_tests.c src/lightmap/lm_material.c src/lightmap/lm_mesh_bake.c src/lightmap/lm_svo_voxelize.c src/lightmap/lm_gi_gather.c src/lightmap/lm_parallel.c src/lightmap/lm_mesh_luxel.c src/lightmap/lm_image.c src/lightmap/lm_farfield.c src/lightmap/lm_sky.c src/lightmap/lm_svo_mip.c src/lightmap/lm_svo_material.c src/lightmap/lm_kdtree.c src/lightmap/lm_solve.c src/lightmap/lm_visibility.c src/lightmap/lm_light.c src/lightmap/lm_atlas.c src/lightmap/lm_lightmap.c src/lightmap/lm_sh.c src/math/vec3.c src/memory/arena_init.c src/memory/arena_alloc.c src/memory/arena_mark.c src/memory/arena_pop.c src/npc/nav/npc_svo_init.c src/npc/nav/npc_svo_rasterize.c src/npc/nav/npc_svo_blocker.c include/ferrum/lightmap/lm_mesh_bake.h | build
	$(CC) $(CFLAGS) tests/lightmap/lm_mesh_bake_tests.c src/lightmap/lm_material.c src/lightmap/lm_mesh_bake.c src/lightmap/lm_svo_voxelize.c src/lightmap/lm_gi_gather.c src/lightmap/lm_parallel.c src/lightmap/lm_mesh_luxel.c src/lightmap/lm_image.c src/lightmap/lm_farfield.c src/lightmap/lm_sky.c src/lightmap/lm_svo_mip.c src/lightmap/lm_svo_material.c src/lightmap/lm_kdtree.c src/lightmap/lm_solve.c src/lightmap/lm_visibility.c src/lightmap/lm_light.c src/lightmap/lm_atlas.c src/lightmap/lm_lightmap.c src/lightmap/lm_sh.c src/math/vec3.c src/memory/arena_init.c src/memory/arena_alloc.c src/memory/arena_mark.c src/memory/arena_pop.c src/npc/nav/npc_svo_init.c src/npc/nav/npc_svo_rasterize.c src/npc/nav/npc_svo_blocker.c src/lightmap/gpu/lm_gpu_gather.c src/lightmap/gpu/lm_gpu_voxelize.c src/lightmap/gpu/lm_gpu_voxelize_util.c src/lightmap/gpu/lm_gpu_voxvol.c src/lightmap/gpu/lm_gpu_voxraster.c src/lightmap/gpu/lm_gpu_voxsample.c src/lightmap/gpu/lm_gpu_voxscene.c src/lightmap/gpu/lm_gpu_chunk_build.c src/lightmap/lm_sdf_file.c src/lightmap/gpu/lm_gpu_pack.c src/lightmap/gpu/lm_gpu_gather_chunked.c src/lightmap/lm_chunk_svo.c src/renderer/chunk/chunk_tree_build.c src/renderer/chunk/chunk_tree_query.c src/renderer/chunk/chunk_grid_init.c src/renderer/chunk/chunk_grid_index.c src/renderer/chunk/chunk_grid_bounds.c -o $@ -ldl -lm
...
build/lm_gather_cmp: tests/visual/lm_gather_cmp.c src/lightmap/gpu/lm_gpu_gather.c src/lightmap/gpu/lm_gpu_voxelize.c src/lightmap/gpu/lm_gpu_voxelize_util.c src/lightmap/gpu/lm_gpu_voxvol.c src/lightmap/gpu/lm_gpu_voxraster.c src/lightmap/gpu/lm_gpu_voxsample.c src/lightmap/gpu/lm_gpu_voxscene.c src/lightmap/gpu/lm_gpu_chunk_build.c src/lightmap/gpu/lm_gpu_gather_chunked.c src/renderer/chunk/chunk_tree_build.c src/renderer/chunk/chunk_tree_query.c src/renderer/chunk/chunk_grid_init.c src/lightmap/lm_chunk_svo.c src/renderer/chunk/chunk_grid_index.c src/renderer/chunk/chunk_grid_bounds.c src/lightmap/lm_sdf_file.c src/lightmap/gpu/lm_gpu_pack.c src/lightmap/lm_material.c src/lightmap/lm_mesh_bake.c src/lightmap/lm_svo_voxelize.c src/lightmap/lm_gi_gather.c src/lightmap/lm_parallel.c src/lightmap/lm_mesh_luxel.c src/lightmap/lm_image.c src/lightmap/lm_farfield.c src/lightmap/lm_sky.c src/lightmap/lm_svo_mip.c src/lightmap/lm_svo_material.c src/lightmap/lm_kdtree.c src/lightmap/lm_solve.c src/lightmap/lm_visibility.c src/lightmap/lm_light.c src/lightmap/lm_atlas.c src/lightmap/lm_lightmap.c src/lightmap/lm_sh.c src/memory/arena_init.c src/memory/arena_alloc.c src/memory/arena_mark.c src/memory/arena_pop.c src/npc/nav/npc_svo_init.c src/npc/nav/npc_svo_rasterize.c src/npc/nav/npc_svo_blocker.c src/renderer/pbr_shader.c src/renderer/material.c src/renderer/texture_create.c src/renderer/texture_upload.c src/renderer/texture_bind.c src/renderer/shader_program_create.c src/renderer/shader_program_bind.c src/renderer/shader_program_destroy.c src/renderer/shader_program_handle.c src/renderer/shader_uniforms_init.c src/renderer/gl_loader_validate.c src/math/mat4_proj.c src/math/mat4_look_at.c src/math/mat4_mul.c src/math/mat4_basic.c src/math/vec3.c $(OBJ_GLAD) src/renderer/render_forward.c src/renderer/shadow_cube.c src/renderer/shadow_spot.c src/renderer/shadow_csm_init.c src/renderer/shadow_csm_cascade.c src/renderer/shadow_csm_render.c src/renderer/shadow_csm_mask.c src/renderer/shadow_caustics_init.c src/renderer/shadow_caustics_bake.c src/renderer/render_forward_caustic_sdf.c src/renderer/shadow_csm_bind.c src/renderer/shadow_csm_blur.c src/renderer/resource/shadow_atlas.c src/renderer/resource/shadow_slotmap.c src/renderer/resource/gpu_registry_create.c src/renderer/resource/gpu_registry_access.c src/memory/pool_alloc.c src/memory/pool_destroy.c src/memory/pool_free.c src/memory/pool_get.c src/memory/pool_init.c src/memory/arena_reset.c src/memory/apool_alloc.c src/memory/apool_init.c src/memory/apool_get.c src/memory/apool_free.c src/memory/apool_destroy.c src/renderer/resource/gpu_cmd_queue.c src/renderer/resource/gpu_executor.c $(JOB_SRC) src/renderer/cull/frustum_cull.c src/renderer/cull/sphere_cull.c src/renderer/depth_prepass.c src/renderer/render_pipeline_graph.c src/renderer/forward_plus.c src/renderer/cluster_grid.c src/renderer/render_camera.c src/renderer/render_scene.c $(RENDER_WORLD_SRC) src/renderer/gi/gi_runtime.c src/renderer/gi/gi_brick_gpu.c src/renderer/gi/gi_runtime_bricks.c src/renderer/gi/gi_probe_gpu_dyn.c src/renderer/gi/gi_runtime_static.c src/renderer/gi/gi_static_volume.c src/renderer/gi/gi_vis_prepass.c src/renderer/gi/gi_sdf_stream.c src/renderer/gi/gi_probe_gpu.c src/renderer/gi/gi_probe_gpu_active.c src/renderer/gi/gi_probe_tuning.c src/renderer/gi/gi_probe_set.c src/renderer/gi/gi_probe_grid.c src/renderer/gi/gi_probe_place.c src/renderer/gi/gi_sdf.c src/renderer/light_store.c src/mesh/obj_mesh_load.c src/mesh/dmesh_load.c src/renderer/mesh/static_mesh_create.c src/renderer/mesh/static_mesh_destroy.c src/renderer/mesh/static_mesh_draw.c src/renderer/mesh/static_mesh_primitives.c src/renderer/vao_create.c src/renderer/vao_bind_attributes.c src/renderer/vao_destroy.c src/renderer/vbo_create.c src/renderer/vbo_upload.c src/renderer/vbo_destroy.c src/math/mat4_inverse.c | build
	$(CC) $(CFLAGS) $(RENDERER_TEST_CFLAGS) tests/visual/lm_gather_cmp.c src/lightmap/gpu/lm_gpu_gather.c src/lightmap/gpu/lm_gpu_voxelize.c src/lightmap/gpu/lm_gpu_voxelize_util.c src/lightmap/gpu/lm_gpu_voxvol.c src/lightmap/gpu/lm_gpu_voxraster.c src/lightmap/gpu/lm_gpu_voxsample.c src/lightmap/gpu/lm_gpu_voxscene.c src/lightmap/gpu/lm_gpu_chunk_build.c src/lightmap/gpu/lm_gpu_gather_chunked.c src/renderer/chunk/chunk_tree_build.c src/renderer/chunk/chunk_tree_query.c src/renderer/chunk/chunk_grid_init.c src/lightmap/lm_chunk_svo.c src/renderer/chunk/chunk_grid_index.c src/renderer/chunk/chunk_grid_bounds.c src/lightmap/lm_sdf_file.c src/lightmap/gpu/lm_gpu_pack.c src/lightmap/lm_material.c src/lightmap/lm_mesh_bake.c src/lightmap/lm_svo_voxelize.c src/lightmap/lm_gi_gather.c src/lightmap/lm_parallel.c src/lightmap/lm_mesh_luxel.c src/lightmap/lm_image.c src/lightmap/lm_farfield.c src/lightmap/lm_sky.c src/lightmap/lm_svo_mip.c src/lightmap/lm_svo_material.c src/lightmap/lm_kdtree.c src/lightmap/lm_solve.c src/lightmap/lm_visibility.c src/lightmap/lm_light.c src/lightmap/lm_atlas.c src/lightmap/lm_lightmap.c src/lightmap/lm_sh.c src/memory/arena_init.c src/memory/arena_alloc.c src/memory/arena_mark.c src/memory/arena_pop.c src/npc/nav/npc_svo_init.c src/npc/nav/npc_svo_rasterize.c src/npc/nav/npc_svo_blocker.c src/renderer/pbr_shader.c src/renderer/material.c src/renderer/texture_create.c src/renderer/texture_upload.c src/renderer/texture_bind.c src/renderer/shader_program_create.c src/renderer/shader_program_bind.c src/renderer/shader_program_destroy.c src/renderer/shader_program_handle.c src/renderer/shader_uniforms_init.c src/renderer/gl_loader_validate.c src/math/mat4_proj.c src/math/mat4_look_at.c src/math/mat4_mul.c src/math/mat4_basic.c src/math/vec3.c $(OBJ_GLAD) src/renderer/render_forward.c src/renderer/shadow_cube.c src/renderer/shadow_spot.c src/renderer/shadow_csm_init.c src/renderer/shadow_csm_cascade.c src/renderer/shadow_csm_render.c src/renderer/shadow_csm_mask.c src/renderer/shadow_caustics_init.c src/renderer/shadow_caustics_bake.c src/renderer/render_forward_caustic_sdf.c src/renderer/shadow_csm_bind.c src/renderer/shadow_csm_blur.c src/renderer/resource/shadow_atlas.c src/renderer/resource/shadow_slotmap.c src/renderer/resource/gpu_registry_create.c src/renderer/resource/gpu_registry_access.c src/memory/pool_alloc.c src/memory/pool_destroy.c src/memory/pool_free.c src/memory/pool_get.c src/memory/pool_init.c src/memory/arena_reset.c src/memory/apool_alloc.c src/memory/apool_init.c src/memory/apool_get.c src/memory/apool_free.c src/memory/apool_destroy.c src/renderer/resource/gpu_cmd_queue.c src/renderer/resource/gpu_executor.c $(JOB_SRC) src/renderer/cull/frustum_cull.c src/renderer/cull/sphere_cull.c src/renderer/depth_prepass.c src/renderer/render_pipeline_graph.c src/renderer/forward_plus.c src/renderer/cluster_grid.c src/renderer/render_camera.c src/renderer/render_scene.c $(RENDER_WORLD_SRC) src/renderer/gi/gi_runtime.c src/renderer/gi/gi_brick_gpu.c src/renderer/gi/gi_runtime_bricks.c src/renderer/gi/gi_probe_gpu_dyn.c src/renderer/gi/gi_runtime_static.c src/renderer/gi/gi_static_volume.c src/renderer/gi/gi_vis_prepass.c src/renderer/gi/gi_sdf_stream.c src/renderer/gi/gi_probe_gpu.c src/renderer/gi/gi_probe_gpu_active.c src/renderer/gi/gi_probe_tuning.c src/renderer/gi/gi_probe_set.c src/renderer/gi/gi_probe_grid.c src/renderer/gi/gi_probe_place.c src/renderer/gi/gi_sdf.c src/renderer/light_store.c src/mesh/obj_mesh_load.c src/mesh/dmesh_load.c src/renderer/mesh/static_mesh_create.c src/renderer/mesh/static_mesh_destroy.c src/renderer/mesh/static_mesh_draw.c src/renderer/mesh/static_mesh_primitives.c src/renderer/vao_create.c src/renderer/vao_bind_attributes.c src/renderer/vao_destroy.c src/renderer/vbo_create.c src/renderer/vbo_upload.c src/renderer/vbo_destroy.c src/math/mat4_inverse.c -o $@ $(RENDERER_TEST_LIBS) -ldl -lm


build/lm_denoise_tests: tests/lightmap/lm_denoise_tests.c $(LM_DENOISE_SRC) include/ferrum/lightmap/lm_denoise.h | build
	$(CC) $(CFLAGS) tests/lightmap/lm_denoise_tests.c $(LM_DENOISE_SRC) -o $@ $(LDFLAGS)
...
LM_HALL_BAKE_SRC := src/mesh/dmesh_load.c src/mesh/obj_mesh_load.c \
  src/lightmap/lm_lightmap_file.c $(LM_DENOISE_SRC) src/lightmap/lm_material.c src/lightmap/lm_mesh_bake.c \
  ...
  src/lightmap/gpu/lm_gpu_gather.c src/lightmap/gpu/lm_gpu_voxelize.c src/lightmap/gpu/lm_gpu_voxelize_util.c src/lightmap/gpu/lm_gpu_voxvol.c src/lightmap/gpu/lm_gpu_voxraster.c src/lightmap/gpu/lm_gpu_voxsample.c src/lightmap/gpu/lm_gpu_voxscene.c src/lightmap/gpu/lm_gpu_chunk_build.c \
  src/lightmap/gpu/lm_gpu_gather_chunked.c src/lightmap/gpu/lm_gpu_pack.c src/lightmap/lm_sdf_file.c \
  src/renderer/chunk/chunk_tree_build.c src/renderer/chunk/chunk_tree_query.c \
  src/renderer/chunk/chunk_grid_init.c src/renderer/chunk/chunk_grid_index.c src/renderer/chunk/chunk_grid_bounds.c \
  src/lightmap/lm_chunk_svo.c src/lightmap/lm_bake_driver.c \
  src/math/vec3.c
...
build/hall_forward_pipeline: tests/visual/hall_forward_pipeline.c src/renderer/render_forward.c src/renderer/shadow_cube.c src/renderer/shadow_spot.c src/renderer/shadow_csm_init.c src/renderer/shadow_csm_cascade.c src/renderer/shadow_csm_render.c src/renderer/shadow_csm_mask.c src/renderer/shadow_caustics_init.c src/renderer/shadow_caustics_bake.c src/renderer/render_forward_caustic_sdf.c src/renderer/shadow_csm_bind.c src/renderer/shadow_csm_blur.c src/renderer/resource/shadow_atlas.c src/renderer/resource/shadow_slotmap.c src/renderer/resource/gpu_registry_create.c src/renderer/resource/gpu_registry_access.c src/memory/pool_alloc.c src/memory/pool_destroy.c src/memory/pool_free.c src/memory/pool_get.c src/memory/pool_init.c src/renderer/cull/frustum_cull.c src/renderer/cull/sphere_cull.c src/renderer/depth_prepass.c src/renderer/render_pipeline_graph.c src/renderer/forward_plus.c src/renderer/cluster_grid.c src/renderer/render_camera.c src/renderer/render_scene.c $(RENDER_WORLD_SRC) src/renderer/gi/gi_runtime.c src/renderer/gi/gi_brick_gpu.c src/renderer/gi/gi_runtime_bricks.c src/renderer/gi/gi_probe_gpu_dyn.c src/renderer/gi/gi_runtime_static.c src/renderer/gi/gi_static_volume.c src/renderer/gi/gi_vis_prepass.c src/renderer/gi/gi_sdf_stream.c src/renderer/gi/gi_probe_gpu.c src/renderer/gi/gi_probe_gpu_active.c src/renderer/gi/gi_probe_tuning.c src/renderer/gi/gi_probe_set.c src/renderer/gi/gi_probe_grid.c src/renderer/gi/gi_probe_place.c src/renderer/gi/gi_sdf.c src/lightmap/lm_sdf_file.c src/renderer/light_store.c src/mesh/obj_mesh_load.c src/renderer/mesh/static_mesh_create.c src/renderer/mesh/static_mesh_destroy.c src/renderer/mesh/static_mesh_draw.c src/renderer/vao_create.c src/renderer/vao_bind_attributes.c src/renderer/vao_destroy.c src/renderer/vbo_create.c src/renderer/vbo_upload.c src/renderer/vbo_destroy.c src/renderer/pbr_shader.c src/renderer/material.c src/renderer/texture_create.c src/renderer/texture_upload.c src/renderer/texture_bind.c src/renderer/shader_program_create.c src/renderer/shader_program_bind.c src/renderer/shader_program_destroy.c src/renderer/shader_program_handle.c src/renderer/shader_uniforms_init.c src/renderer/gl_loader_validate.c src/math/mat4_proj.c src/math/mat4_look_at.c src/math/mat4_mul.c src/math/mat4_inverse.c src/math/mat4_basic.c src/math/vec3.c $(OBJ_GLAD) | build
	$(CC) $(CFLAGS) -D_POSIX_C_SOURCE=200809L $(RENDERER_TEST_CFLAGS) tests/visual/hall_forward_pipeline.c src/renderer/render_forward.c src/renderer/shadow_cube.c src/renderer/shadow_spot.c src/renderer/shadow_csm_init.c src/renderer/shadow_csm_cascade.c src/renderer/shadow_csm_render.c src/renderer/shadow_csm_mask.c src/renderer/shadow_caustics_init.c src/renderer/shadow_caustics_bake.c src/renderer/render_forward_caustic_sdf.c src/renderer/shadow_csm_bind.c src/renderer/shadow_csm_blur.c src/renderer/resource/shadow_atlas.c src/renderer/resource/shadow_slotmap.c src/renderer/resource/gpu_registry_create.c src/renderer/resource/gpu_registry_access.c src/memory/pool_alloc.c src/memory/pool_destroy.c src/memory/pool_free.c src/memory/pool_get.c src/memory/pool_init.c src/renderer/cull/frustum_cull.c src/renderer/cull/sphere_cull.c src/renderer/depth_prepass.c src/renderer/render_pipeline_graph.c src/renderer/forward_plus.c src/renderer/cluster_grid.c src/renderer/render_camera.c src/renderer/render_scene.c $(RENDER_WORLD_SRC) src/renderer/gi/gi_runtime.c src/renderer/gi/gi_brick_gpu.c src/renderer/gi/gi_runtime_bricks.c src/renderer/gi/gi_probe_gpu_dyn.c src/renderer/gi/gi_runtime_static.c src/renderer/gi/gi_static_volume.c src/renderer/gi/gi_vis_prepass.c src/renderer/gi/gi_sdf_stream.c src/renderer/gi/gi_probe_gpu.c src/renderer/gi/gi_probe_gpu_active.c src/renderer/gi/gi_probe_tuning.c src/renderer/gi/gi_probe_set.c src/renderer/gi/gi_probe_grid.c src/renderer/gi/gi_probe_place.c src/renderer/gi/gi_sdf.c src/lightmap/lm_sdf_file.c src/renderer/light_store.c src/mesh/obj_mesh_load.c src/renderer/mesh/static_mesh_create.c src/renderer/mesh/static_mesh_destroy.c src/renderer/mesh/static_mesh_draw.c src/renderer/vao_create.c src/renderer/vao_bind_attributes.c src/renderer/vao_destroy.c src/renderer/vbo_create.c src/renderer/vbo_upload.c src/renderer/vbo_destroy.c src/renderer/pbr_shader.c src/renderer/material.c src/renderer/texture_create.c src/renderer/texture_upload.c src/renderer/texture_bind.c src/renderer/shader_program_create.c src/renderer/shader_program_bind.c src/renderer/shader_program_destroy.c src/renderer/shader_program_handle.c src/renderer/shader_uniforms_init.c src/renderer/gl_loader_validate.c src/math/mat4_proj.c src/math/mat4_look_at.c src/math/mat4_mul.c src/math/mat4_inverse.c src/math/mat4_basic.c src/math/vec3.c $(OBJ_GLAD) -o $@ $(RENDERER_TEST_LIBS) -ldl -lm

# --- rpg-xkdz: hall lightmapped + dynamic (SH lightmap through forward+) ---
build/hall_lit_dynamic: tests/visual/hall_lit_dynamic.c src/renderer/render_forward.c src/renderer/shadow_cube.c src/renderer/shadow_spot.c src/renderer/shadow_csm_init.c src/renderer/shadow_csm_cascade.c src/renderer/shadow_csm_render.c src/renderer/shadow_csm_mask.c src/renderer/shadow_caustics_init.c src/renderer/shadow_caustics_bake.c src/renderer/render_forward_caustic_sdf.c src/renderer/shadow_csm_bind.c src/renderer/shadow_csm_blur.c src/renderer/resource/shadow_atlas.c src/renderer/resource/shadow_slotmap.c src/renderer/resource/gpu_registry_create.c src/renderer/resource/gpu_registry_access.c src/memory/pool_alloc.c src/memory/pool_destroy.c src/memory/pool_free.c src/memory/pool_get.c src/memory/pool_init.c src/memory/arena_alloc.c src/memory/arena_init.c src/memory/arena_mark.c src/memory/arena_pop.c src/memory/arena_reset.c src/memory/apool_alloc.c src/memory/apool_init.c src/memory/apool_get.c src/memory/apool_free.c src/memory/apool_destroy.c src/renderer/resource/gpu_cmd_queue.c src/renderer/resource/gpu_executor.c $(JOB_SRC) src/renderer/cull/frustum_cull.c src/renderer/cull/sphere_cull.c src/renderer/depth_prepass.c src/renderer/render_pipeline_graph.c src/renderer/forward_plus.c src/renderer/cluster_grid.c src/renderer/render_camera.c src/renderer/render_scene.c $(RENDER_WORLD_SRC) src/renderer/gi/gi_runtime.c src/renderer/gi/gi_brick_gpu.c src/renderer/gi/gi_runtime_bricks.c src/renderer/gi/gi_probe_gpu_dyn.c src/renderer/gi/gi_runtime_static.c src/renderer/gi/gi_static_volume.c src/renderer/gi/gi_vis_prepass.c src/renderer/gi/gi_sdf_stream.c src/renderer/gi/gi_probe_gpu.c src/renderer/gi/gi_probe_gpu_active.c src/renderer/gi/gi_probe_tuning.c src/renderer/gi/gi_probe_set.c src/renderer/gi/gi_probe_grid.c src/renderer/gi/gi_probe_place.c src/renderer/gi/gi_sdf.c src/lightmap/lm_sdf_file.c src/renderer/light_store.c src/mesh/obj_mesh_load.c src/mesh/dmesh_load.c src/renderer/mesh/static_mesh_create.c src/renderer/mesh/static_mesh_destroy.c src/renderer/mesh/static_mesh_draw.c src/renderer/mesh/static_mesh_primitives.c src/renderer/vao_create.c src/renderer/vao_bind_attributes.c src/renderer/vao_destroy.c src/renderer/vbo_create.c src/renderer/vbo_upload.c src/renderer/vbo_destroy.c src/renderer/pbr_shader.c src/renderer/material.c src/renderer/texture_create.c src/renderer/texture_upload.c src/renderer/texture_bind.c src/renderer/shader_program_create.c src/renderer/shader_program_bind.c src/renderer/shader_program_destroy.c src/renderer/shader_program_handle.c src/renderer/shader_uniforms_init.c src/renderer/gl_loader_validate.c src/math/mat4_proj.c src/math/mat4_look_at.c src/math/mat4_mul.c src/math/mat4_inverse.c src/math/mat4_basic.c src/math/vec3.c $(OBJ_GLAD) | build
