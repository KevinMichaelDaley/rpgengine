...
static const char *const PBR_FS =
    ...
    "  float shr[4]; float shg[4]; float shb[4];\n"
    "  for(int k=0;k<4;++k){ shr[k]=0.0; shg[k]=0.0; shb[k]=0.0; }\n"
    ...
    "    int base=ni[j]*12;\n"
    "    for(int k=0;k<4;++k){ shr[k]+=texelFetch(u_probe_sh,base+k).r*w;\n"
    "      shg[k]+=texelFetch(u_probe_sh,base+4+k).r*w;\n"
    "      shb[k]+=texelFetch(u_probe_sh,base+8+k).r*w; }\n"
    "  }\n"
    ...
    "  vec3 r;\n"
    "  r.r=3.14159265*shr[0]*y0+2.0943951*(shr[1]*y1+shr[2]*y2+shr[3]*y3);\n"
    "  r.g=3.14159265*shg[0]*y0+2.0943951*(shg[1]*y1+shg[2]*y2+shg[3]*y3);\n"
    "  r.b=3.14159265*shb[0]*y0+2.0943951*(shb[1]*y1+shb[2]*y2+shb[3]*y3);\n"
