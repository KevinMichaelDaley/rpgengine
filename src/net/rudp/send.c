...
int net_rudp_peer_send_unreliable(net_rudp_peer_t *peer,
                                  ...
                                  const void *payload,
                                  size_t payload_size) {
    return net_rudp_peer_send_unreliable_with_sendto(peer, sock, socket_sendto_, to, now_ms, schema_id, payload, payload_size);
}
...
int net_rudp_peer_send_reliable(net_rudp_peer_t *peer,
    ...
    return net_rudp_peer_send_reliable_with_sendto(peer, sock, socket_sendto_, to, now_ms, schema_id, payload, payload_size, out_sequence);
}

int net_rudp_peer_tick_resend(net_rudp_peer_t *peer, net_udp_socket_t *sock, const net_udp_addr_t *to, uint64_t now_ms) {
    return net_rudp_peer_tick_resend_with_sendto(peer, sock, socket_sendto_, to, now_ms);
}
