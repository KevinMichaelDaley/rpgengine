...
static const char *const PBR_FS =
     ...
     * still dark. */
    "vec3 pbr_csm_glass(vec2 uv, int i, float d, float gbias, float prad, mat2 rot){\n"
    "  if(u_csm_mask_on==0) return vec3(1.0);\n"
    "  vec3 t = vec3(0.0);\n"
    "  for(int s=0;s<4;++s)\n"
    "    t += pbr_csm_tap_trans(uv + rot*PZ[s*2]*(prad*0.5), i, d, gbias);\n"
    "  return t * 0.25;\n"
    "}\n"
    ...
    "    }\n"
    "    vec3 lit = vec3(litf) * pbr_csm_glass(uv, i, d, gbias, prad, rot);\n"
