...
static const char *CS_SRC =
    ...
    /* Albedo at a TRACE HIT. The DYNAMIC volume is sampled at the hit point p\n"
     * itself: the old half-voxel inset (meant to bias the STATIC voxel read into\n"
     * the wall) pushed the lookup out of a thin dynamic object's few coarse voxels\n"
     * -- a ray would stop ON the red banner yet read the wall behind it, so the\n"
     * bounce came back stone-grey. Static albedo keeps the inset (its voxels live\n"
     * inside the surface). */
    "vec3 scene_albedo_hit(vec3 p, vec3 n, float lod){\n"
    "  vec4 dc=dyn_alb_at(p); if(dc.a>0.02) return dc.rgb/max(dc.a,1e-3);\n"
    "  return scene_albedo(p - n*u_sdf_vox[0]*0.5, lod); }\n"
    ...
    /* PROBE RELOCATION (DDGI): a lattice probe that lands INSIDE geometry (or\n"
     * pressed against it) traces from inside the SDF, gathers BLACK, and its tiny\n"
     * depth means put nearby fragments inside the vis_bias band -- a fully-visible\n"
     * black probe = the dark vertical scuff down pier faces. Push the TRACE ORIGIN\n"
     * out along the SDF normal (deterministic per position, recomputed identically\n"
     * in every pass); the analytic grid lookup keeps the lattice position. */
    "vec3 relocate(vec3 o){ float sd=scene_sdf(o);\n"
    "  if(sd < 0.10){ vec3 n=sdf_normal(o); o += n*(0.10 - sd + 0.08); }\n"
    "  return o; }\n"
    ...
    "      vec3 alb = scene_albedo_hit(p, n, lod);\n"
    ...
    "void pass_classify(uint gid){ vec3 o=relocate(ppos[gid].xyz);\n"
    ...
    /* SOURCE test: every probe NEAR GEOMETRY is a source. surf_min (the min of the\n"
     * temporally-smoothed octahedral depth) can miss thin or newly-resident\n"
     * geometry for many frames, so also take a DIRECT SDF sample at the probe --\n"
     * exact, includes the dynamic collider boxes (the banner), and one tap. */
    "  float sd0 = scene_sdf(o);\n"
    "  if(surf_min<u_dmax || sd0<u_dmax || elum>u_emin){ uint idx=atomicAdd(acount,1u); if(idx<uint(u_nprobes)) alist[idx]=gid; } }\n"
    ...
    "void pass_gather(uint gid){ if(!in_group(gid)) return; vec3 o=relocate(ppos[gid].xyz);\n"
    ...
    /* SYMMETRIC visibility: receiver's Chebyshev toward the source TIMES the\n"
     * source's toward the receiver. One-sided + the softened bias/varmin (tuned\n"
     * against lattice dots) transmits a fraction through a wall; with exterior\n"
     * probes now resident+lit (12-slot pool, zone fallback, near-SDF sources),\n"
     * that fraction read as light bleeding through the wall base. A wall sits in\n"
     * BOTH depth maps, so the product suppresses quadratically; open-space pairs\n"
     * are ~1*1 and unaffected. */
    "      float vis=probe_vis(gid,dir,dist)*probe_vis(m,-dir,dist); if(vis<0.05) continue;\n"
    ...
    /* ENERGY-CONSERVING gather. Two rules make the recurrence bounded + calibrated:\n"
     *  (1) A source probe is a SURFACE proxy: it re-emits its stored irradiance E\n"
     *      as RADIANCE rho*E/PI (neutral rho 0.5). Feeding E straight in (the old\n"
     *      code) over-injected by ~2*PI and the forward pass cosine-convolved it a\n"
     *      second time -- the too-bright probe diffuse.\n"
     *  (2) Heroes join the SAME weighted average as the field samples (a CONVEX\n"
     *      combination x u_bounce < 1). Adding them ON TOP of the normalised\n"
     *      average pushed the transport gain past u_bounce (even past 1 where\n"
     *      heroes dominate) -- the brightness that ACCUMULATED over time.\n"
     * Hero trace() radiance already includes the true hit albedo/PI, so only the\n"
     * field samples get the re-emission factor. */
    ...
    "  const float REEMIT = 0.5/3.14159265;\n"
    ...
    "      vec3 pm=ppos[m].xyz; vec3 d=pm-o; float dist=length(d); if(dist<0.35) continue; vec3 dir=d/dist;\n"
    "      float vis=probe_vis(gid,dir,dist)*probe_vis(m,-dir,dist); if(vis<0.02) continue;\n"
    "      float ndl=(nv>0.5)? max(dot(dir,nrm),0.0)+0.04 : 1.0;\n"
    "      vec3 Lmd=max(psh_irr(int(m)*24, -dir), vec3(0.0))*REEMIT;\n"
    "      vec3 Lms=max(psh_irr(int(m)*24+12, -dir), vec3(0.0))*REEMIT;\n"
     ...
     * folded into the SAME average (weight = the field formula; the trace itself\n"
     * is the visibility). */
    ...
    "    float ndl=(nv>0.5)? max(dot(dir,nrm),0.0)+0.04 : 1.0;\n"
    "    float wt=ndl*(cellsq/max(dist*dist,cellsq)); wsum+=wt;\n"
    "    float y[4]; sh_basis(dir,y);\n"
    "    if(nrs<32){ rdir[nrs]=dir; rcol[nrs]=(rd+rs)*wt; ++nrs; }\n"
    "    for(int k=0;k<4;++k){ bd[k]+=rd.r*y[k]*wt; bd[4+k]+=rd.g*y[k]*wt; bd[8+k]+=rd.b*y[k]*wt;\n"
    "                          bs[k]+=rs.r*y[k]*wt; bs[4+k]+=rs.g*y[k]*wt; bs[8+k]+=rs.b*y[k]*wt; } }\n"
    /* ONE convex normalisation over field + heroes: transport gain == u_bounce. */
    ...
    "  vec3 o=relocate(ppos[gid].xyz); float shd[12]; float shs[12];\n"
    ...
    "void update_mis(uint gid){ if(!in_group(gid)) return; vec3 o=relocate(ppos[gid].xyz);\n"
